...
OSFを使い始めるには、まず[ウェブサイト](https://osf.io/register)で個人アカウントを作成する必要がある。アカウントが作成されたら、 _R_ を使って直接リポジトリを操作できるように、**アクセストークン**を生成する必要がある。アクセストークンを取得するには、**Profile** &gt; **Settings** &gt; **Personal access tokens**に移動する必要がある。そこで、**Create token** をクリックする。
