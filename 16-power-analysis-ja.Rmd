...
* 見つけたい効果量。これは特に重要で、効果量がどの程度大きければ意味があるのかを仮定しなければならないことがある。例えば、あるうつ病の介入研究では、SMD = 0.24という小さい効果でも患者にとって意味がある可能性があると計算されている [@cuijpers2014threshold] 。介入の負の効果（例えば、死亡や症状の悪化）を研究したい場合、非常に小さな効果量であっても極めて重要であり、検出されるべきものである。
...
分析の**後**に行われる検出力分析（「post hoc 分析」）は、深い欠陥のある論理に基づいている [@hoenig2001abuse] 。まず、post hoc の検出力分析は、**一様**であり、私たちがまだ知らないことは何も教えてくれない。収集したサンプルに基づいて効果が有意でないことがわかったとき、計算された post hoc 検出力は、定義上、不十分（すなわち、50％以下）である。あるテストの post hoc に検出力を計算するとき、単に結果の $p$ 値に直接リンクしている検出力関数で「遊んで」いるにすぎない。
...
効果量をその標準誤差で割ると $z$  スコアになることは、以前すでに取り上げた（ Chapter \@ref(metareg-continuous)  を参照）。$z$  スコアは、標準正規分布に従う。ここで、$|z| \geq$  1.96 という値は、効果がゼロとは有意に異なることを意味する ( $p<$  0.05)。これはまさにメタ分析で達成したいことである。つまり、結果の正確な効果量と標準誤差がどんなに大きくても、$|z|$  の値は少なくとも1.96でなければならず、したがって統計的に有意でなければならないのである。 
...
この式は、標準化平均差の標準誤差を計算するのに使われる式と非常によく似ているが、1つだけ例外がある。ここで、標準誤差を $K$  で割る。これは、プールされた効果の標準誤差が、メタ分析における研究の総数を表す係数 $K$  によって減少することを意味する。言い換えれば、固定効果モデルを仮定した場合、研究をプールすると、全体効果の精度が $K$ -倍になる^[このステートメントは、もちろん、方程式 \@ref(eq:pow3) の簡易式を使用しているため正しいことに注意してみよう]。 
...
個のメタ分析には 10 件の研究が含まれているにもかかわらず、60.6%で、は**検出力不足**であることがわかる。固定効果メタ分析の検出力を計算する、より便利な方法は、`power.analysis` 関数を使用することである。
...
* **`p`**. 使用するアルファレベル。デフォルトは $\alpha$ =0.05。
...
推定された検出力は40.76%であることがわかる。これは、標準的な80%の閾値よりも低い値である。また、固定効果モデルを仮定した場合の60.66%よりも低くなっている。これは、研究間の異質性がプール効果の推定値の精度を低下させ、その結果、統計的検出力が低下するためである。 
...
グループ１の効果は SMD = 0.3、標準誤差は 0.13 であり、グループ２の効果はSMD = 0.66、標準誤差は 0.14 であると仮定しよう。これらの仮定条件を関数の呼び出しの入力として使用することができる。
...
出力では、私たちの想像上のサブグループ検定の検出力 (47%) が十分でないことがわかる。出力は、他のすべてが同じで、十分な検出力に達するには、効果量の差が少なくとも 0.54 である必要があることも教えてくれる。
