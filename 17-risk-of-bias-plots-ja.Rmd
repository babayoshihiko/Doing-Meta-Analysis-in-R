...
プロットを作成するために、まず、**Excel** から _R_ にバイアスリスク評価の結果をインポートする必要がある。なお、 **{robvis}** は提供されたデータについて特定の事実を期待しているので、Excel で表を設定する際には以下のガイダンスに必ず従わなければならない。

1. 最初の列は "Study" とする。研究識別子を含む（例：**Anthony et al, 2019**）。
2. 最後から2番目の列は "Overall" とする。全体のリスクオブバイアス判定が記載されている。
3. 最後の列は "Weight" とする。研究の精度の指標（例えば、メタ分析で各研究に割り当てられた重み、またはメタ分析が行われなかった場合は、各研究のサンプルサイズ）を含んでいる。詳しくは、Chapter \@ref(fem) を参照。
...
* **QUADAS-2**, the Quality and Applicability of Diagnostic Accuracy Studies, Version 2 である。
...
# 要約棒グラフを作成; macOS では文字化けするためフォントして
rob_summary(data = new_rob1_data, tool = "ROB1") + theme_classic(base_family = "Hiragino Kaku Gothic Pro W3")
```
