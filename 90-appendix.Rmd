...
本書に示されたコードを実行する際に問題があった？原因としては、パッケージの**バージョンが古すぎるか新しすぎる**ことがある。この場合、本書で使用したバージョンと同じバージョンをインストールすると成功することがある。R パッケージを、バージョンを指定してインストールする方法は、[ここ](https://support.posit.co/hc/en-us/articles/219949047-Installing-older-versions-of-packages)に係れている。
```
