...
さらにもう1つの問題がある。直接比較から間接的な証拠を推定することができる先ほどの式 \@ref(eq:networkes) は、重要な前提条件である**推移性 (transivity)** の仮定が満たされた場合にのみ成立するのである。統計学的な観点からは、この仮定はネットワークの**一貫性 (consistency)** と訳される（訳注：Minds では「一貫性」、神田は「一致性」と訳している。このほか「整合性」と訳されることもある） [@efthimiou2016getreal]。以下では、この2つの用語の意味と、それらがなぜ重要であるかを説明する。
